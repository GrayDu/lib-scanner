...
class Audit {
  ...
  start() {
    ...
    this.checkOutdate().catch(() => {
      ...
      this.execAudit((url) => {
        ...
        if (url) {
          exec('npm config set registry ' + url)
        }
  ...
  // npm audit
  execAudit (callback) {
    exec('npm get registry', (error, out) => {
      const registry = out;
      if (out !== npmRegistry) {
            ...
            if (result && result.report) {
              ...
              if (data.metadata.vulnerabilities.total > 0) {
                for (let key in data.vulnerabilities) {
                  ...
                  this.auditList.push(data.vulnerabilities[key])
                }
                callback()
              } else {
                callback()
  ...
  handleArr (arr) {
    ...
    arr.forEach(item => {
      ...
      if (!utils.isCaps(item[0])) {
        if (i === 0) {
          newArr[newArr.length - 1].value += (' ' + item)
        } else {
          newArr[newArr.length - 1].value += (' ' + item)
        }
      } else {
